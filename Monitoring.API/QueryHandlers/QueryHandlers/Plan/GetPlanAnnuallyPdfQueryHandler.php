<?php
    ...
    use API\QueryHandlers\Queries\Plan\GetPlanAnnuallyPdfQuery;
    ...
    final class GetPlanAnnuallyPdfQueryHandler extends SecurityFunctionsHandler
    {
        ...
        /**
         * @param GetPlanAnnuallyPdfQuery $query
         */
        public function QueryCommandResult($query)
        {
            ...
            GetPlanAnnuallyPdfQueryHandler::DrawCompanyHeader($pdf, $query->Year, $user);
            ...
            foreach ($areas as $area)
            {
                ...
                $queryDb = $this->pdo->query($objectDao->GetObjectTypesForActiveObjects($area->Id));
                ...
                foreach ($objectTypes as $objectType)
                {
                    ...
                    $queryDb = $this->pdo->query($planDao->GetObjectsForAnnuallyPlanPdf($query->Year, $area->Id, $objectType->Id));
                    ...
                    $pdf->MultiCell(30, $lineHeight, 'MJESEC', 0, 'C');
                    ...
                    foreach ($objects as $object)
                    {
                        ...
                        $queryDb = $this->pdo->query($objectDao->GetMonitoringsCountFromObject($object->ObjectId));
                        ...
                        $planMonths = $queryDb->fetchAll(PDO::FETCH_COLUMN, 0);

                        $yPosition = GetPlanAnnuallyPdfQueryHandler::CheckPageBreak($yPosition, $yPosStart, $pdf, $query->Year, $user, $monitoringsObjectCount, $lineHeight);
                        $pdf->SetFont('DejaVu', '', $mainTextFontSize);

                        //Crtanje pravokutnika redni broj, objekt i mjesec
                        //Brojač
                        ...
                        $pdf->MultiCell(30, 3, implode(", ", $planMonths), 0, 'L');

                        //Dobavi objectIteme
                        $queryDb = $this->pdo->query($objectDao->GetObjectItems($object->ObjectId));
                        ...
                        foreach ($objectItems as $objectItem)
                        {
                            ...
                            $queryDb = $this->pdo->query($objectDao->GetMonitoringsCountFromObjectItem($objectItem->Id));
                            ...
                            //Dobavi monitoringe
                            $queryDb = $this->pdo->query($objectDao->GetObjectItemMonitoringsAndPlans($objectItem->Id));
         ...
         * @param tFPDF $pdf
         */
        private static function DrawCompanyHeader($pdf, $year, $user)
        {
            ...
            $pdf->Text($xPosStart + 125.0, $yPosStart + 9.0, 'Oznaka: V1');
            ...
            $pdf->MultiCell(100, 6, 'GODIŠNJI PLAN UZORKOVANJA ZA ' .$year, 0, 'C');

            //Labela na dnu
            $pdf->Text($xPosStart, $yPosStart + 270, 'PI-5.7.-V.1./02');

            //Poziv footera samo na prvoj stranici
            if($pdf->PageNo() == 1)
            {
                GetPlanAnnuallyPdfQueryHandler::DrawFirstPageFooter($pdf, $user);
            }
        }

        private static function CheckPageBreak($yPosition, $yPosStart, $pdf, $year, $user, $monitoringsObjectCount, $lineHeight)
        {
            $limit = 259;
            if($pdf->PageNo() == 1)
            {
            ...
            {
                $pdf->AddPage();
                GetPlanAnnuallyPdfQueryHandler::DrawCompanyHeader($pdf, $year, $user);
                return $yPosStart;
            }
            return $yPosition;
        }
    ...
    QueryCommandController::Respond(
        new GetPlanAnnuallyPdfQueryHandler($_SERVER['HTTP_AUTHORIZATIONTOKEN']),
        new GetPlanAnnuallyPdfQuery(json_decode(file_get_contents("php://input"), true)));